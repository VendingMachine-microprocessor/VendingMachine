...
volatile int piezo_flag = 0;
volatile int INT0_flag = 0;
volatile int INT1_flag = 0;
volatile int INT2_flag = 0;
volatile int INT3_flag = 0;
...
void loop() {
  ...
  // 500원짜리 1개
  if ((Number == 1) && (weight > 5.5) && (weight < 7.0)) {
    cost = 500;
  } 
  ...
  // 500원짜리 2개
  if ((Number == 2) && (weight > 11.0) && (weight < 12.5)) {
    cost = 1000;
  } 

  // 500원짜리 1개 + 100원짜리 5개
  if ((Number == 6) && (weight > 24.5) && (weight < 27.5)) {
    cost = 1000;
  } 
